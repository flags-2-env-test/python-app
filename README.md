...
A Python 3.12 / ctypes consumer of
...
```bash
git clone --recurse-submodules https://github.com/flags-2-env-test/python-app
cd python-app
docker build -t f2e-fixture-python .
docker run --rm f2e-fixture-python
```
...
docker run --rm -v /tmp/mutant.toml:/app/.cli-flags.toml:ro f2e-fixture-python
